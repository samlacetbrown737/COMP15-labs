...
void function0(int n); // O(n^2)
void function1(int n); // O(1)
void function2(int n); // O(n)
void function3(int *arr, int n); // O(1)
void function4(int *arr, int n); // O(1)
void function5(int *arr, int n); // O(log n)
void function6(int *arr, int n); // O(n log n)
bool search(int *arr, int n, int target, int current_index); // O(n)
int findMax(int *arr, int n); // O(n)
int uselessFunction(int *arr, int n); //O (n^2)
// example driver for main function.
// test cases would have to be self written.
// just an example driver, to see function calls.
...
int main() {
    ...
    search(someArray, someN,  232122,  0);
