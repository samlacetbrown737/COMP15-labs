 ...
 * Contains: Procedures to "print a tree". That is, given a pointer
 *           to a node, displays the induced tree in out 
...
/*
 * Pretty formatting of a binary tree to the output stream @ param
 * level Control how wide you want the tree to sparse (eg, level 1 has
 * the minimum space between nodes, while level 2 has a larger space
 * between nodes) indentSpace Change this to add some indent space to
 * the left (eg, indentSpace of 0 means the lowest level of the left
 * node will stick to the left margin)
*/
